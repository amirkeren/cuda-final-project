...
template<> matrix_class::storage_scheme str_to_enum<matrix_class::storage_scheme>(const string& lower_case_str)
{
...
static void check_header(istream& is, const string& target_value)
{
  ...
  if (is >> value)
  {
	  ...
	  {
		  throw std::runtime_error("Invalid value in header: " + value + " != " + target_value);
	  }
  }
  else
  {
...
template <typename enum_type> enum_type get_header_value(istream& is)
{
  string value;
  if (is >> value)
  {
...
matrix_class::matrix_meta read_meta(string& meta_line, matrix_class::format format, matrix_class::data_type data_type, matrix_class::storage_scheme storage_scheme)
{
...
template <typename data_t> class sparse_matrix : public matrix_class
{
public:
  sparse_matrix (matrix_class::matrix_meta meta_arg, unique_ptr<data_t[]> data_arg, unique_ptr<unsigned int[]> row_ids_arg, unique_ptr<unsigned int[]> col_ids_arg): matrix_class(meta_arg), data(move(data_arg)), row_ids(move(row_ids_arg)), col_ids(move(col_ids_arg)) {}
...
public:
  data_reader() = delete;
  explicit data_reader(matrix_class::matrix_meta meta_arg): meta(meta_arg) {}
...
{
public:
  explicit sparse_reader(matrix_class::matrix_meta meta_arg): data_reader(meta_arg) {
	if (!meta.is_sparse())
	{
...
private:
  template <typename data_t> unique_ptr<sparse_matrix<data_t>> read_lines(istream& is, unique_ptr<data_t[]>& data)
  {
    string line;
    size_t nz = 0;
    ...
    while (getline (is, line))
    {
      ...
      iss >> row_ids[nz] >> col_ids[nz] >> data[nz];
             row_ids[nz]--; col_ids[nz]--;
      nz++;
    }

    return make_unique<sparse_matrix<data_t>>(meta, move(data), move(row_ids), move(col_ids));
  }
