...
public:
  ...
  measurement_class(std::string format, double elapsed, double load_store_bytes, double operations_count);
...
template <typename data_type> void compare_results (unsigned int y_size, const data_type* a, const data_type* b)
{
...
void internal_resize(char*& data, std::size_t new_size);
void internal_free(char*& data);

template <typename T> class resizable_gpu_memory
{
public:
